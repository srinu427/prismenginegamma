...
#define SMAP_MUL_EPSILON 1.00
#define SMAP_ADD_EPSILON 0.0001
#define SMAP_BLUR_EPSILON 0.00005
#define SMAP_BLUR_SCALING 20
#define SMAP_BLUR_FALLOFF 0.6
#define SMAP_BLUR_SAMPLES 12

#define SCUBE_MUL_EPSILON 1.00
...
vec3 norm_vec_in_dir(vec3 v, vec3 dir){
	if (length(dir) == 0) return v;
	vec3 ndir = normalize(dir);
	vec3 tmpo =  v + (-dot(ndir, v)) * ndir;
	tmpo = tmpo + (-dot(ndir, tmpo)) * ndir;
	return tmpo;
}
...
float compute_shadow(int lidx, vec3 light_vec){
	...
	float sample_weight = SCUBE_BLUR_FALLOFF/(SCUBE_BLUR_SCALING*brad+1);
...
float compute_map_shadow(int lidx, vec3 light_vec, vec4 ppos){
	...
	if(texture(shadowMaps[midx], csample).r*SMAP_MUL_EPSILON >= lvl + 0.0001){light_samples += 1;}
	vec2 sample_vec;
	float brad = SMAP_BLUR_EPSILON*sqrlvl;
	vec4 v1 = brad*vec4(10, 0, 0, 0), v2 = brad*vec4(0, 10, 0, 0);
	float sample_weight = SMAP_BLUR_FALLOFF/(SMAP_BLUR_SCALING*brad+1);
	float theta = PI/SMAP_BLUR_SAMPLES;
	for (int i = 0; i < SMAP_BLUR_SAMPLES; i++){
		sample_vec = v1.xy*smap_cosines[i] + v2.xy*smap_sines[i];
		if(texture(shadowMaps[midx], csample + sample_vec).r*SMAP_MUL_EPSILON >= lvl){light_samples += sample_weight;}
		if(texture(shadowMaps[midx], csample - sample_vec).r*SMAP_MUL_EPSILON >= lvl){light_samples += sample_weight;}
		sample_vec = v1.xy*smap_cosines[i] - v2.xy*smap_sines[i];
		if(texture(shadowMaps[midx], csample + sample_vec).r*SMAP_MUL_EPSILON >= lvl){light_samples += sample_weight;}
		if(texture(shadowMaps[midx], csample - sample_vec).r*SMAP_MUL_EPSILON >= lvl){light_samples += sample_weight;}
	}
	float sprob = (light_samples/(1 + 4*SMAP_BLUR_SAMPLES*sample_weight));
...
void main() {
    ...
    //vec4 texColor = vec4(0.5,0.5,0.5,0);
	...
	for (int i = 0; i < MAX_PLIGHTS + MAX_DLIGHTS; i++){
		...
		if (bool(lightBuffer.lights[i].flags.x & 1)){
			if (i < MAX_PLIGHTS){
				...
				shadow *= compute_map_shadow(i, lightVec, vec4(fragPosition, 1.0f));
			}
		}
